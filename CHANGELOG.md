...
- New quote provider **Alpha** (id `zap2`): the Rust zapper, quoted through the Reserve API's `api/zapper2/{chain}/swap` endpoint (same query string and response contract as `zap`, so it goes through the same `zapperApiUrl`). Enabled on Ethereum, Base and BSC; it shows as "Alpha" with an α icon in the route list and competes like an external venue: `zap` keeps the tie-break preference, `zap2` gets none. `ProviderId` gains the `'zap2'` member; hosts that build exhaustive `Record<ProviderId, …>` maps need the new key.
